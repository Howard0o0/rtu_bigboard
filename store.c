...
int  Store_ReadDataItem(char _index , char * _dest, int _flag)
{
    ...
    {//重试3次
        ...
        {
            TraceMsg("Store_ReadAnalogSelect failed",1);
            return -1;
        }
        //添加模拟量
        for(int i=0;i<8;++i)
        ...
        {
           TraceMsg("Store_ReadPulseSelect failed",1);
           return -1;
        }
        for(int i=0;i<4;++i)
        {
        ...
        {
            TraceMsg("Store_ReadIoSelect failed",1);
            return -1;
        }
        for(int i=0;i<8;++i)
        {//对于8个位
...
int  New_Store_ReadDataItem(char _index , char * _dest, int _flag)
{
    if( _index < DATA_MIN_IDX || _index > DATA_MAX_IDX)
        return -2;
    int _addr=DATA_START_ADDR + (_index -1)*DATA_ITEM_LEN; //
    char _tempChar1=0x00;
    char _tempChar2=0x00;
    unsigned short _contextid=0xff00;
    unsigned int  _tempInt=0; 
    char _buffer[50];
    int  _repeats=0;
    int  _idx1=0;//_buffer的索引
     ...
     //   _idx2=10;//后续使用前缀++
        ...
        for(int i=0;i<8;++i)
        {
            ...
            {//如果该位有
                ...
                contextlen+=5;
                _contextid++;
                _dest[_idx2++] = _contextid>>8;
                _dest[_idx2++]= _contextid&0xff;
                _dest[_idx2++]= 0x20;    //4bit 00100 000  前5位是小数点前的值.为了适应新测试,小数点确定让服务器来决定
                _tempInt = (((unsigned int)_buffer[_idx1 + 2*i])<<8) + _buffer[_idx1 + 2*i+1];
                IntTo0xInt(_tempInt,4); 
                _dest[_idx2++]=anahigh;
                _dest[_idx2++]=analow;
                if(i==0){
                    vthigh=anahigh;
        ...
        for(int i=0;i<4;++i)
        {
            ...
            {//如果该位有才添加该位
                ...
                _dest[_idx2++] = _contextid>>8;
                _dest[_idx2++]= _contextid&0xff;
                _dest[_idx2++]=0x30;     //6bit   00110 000
              //  _dest[++ _idx2] = 'I' + i ;
              //  ++ _idx2;
              //  Utility_CharToHex( _buffer[_idx1+3*i],&(_dest[_idx2]));
             //   _idx2+=2;//函数填写2位.
           ...
           //     Utility_CharToHex( _buffer[_idx1+3*i+2],&(_dest[_idx2]));
           //     _idx2+=1; //因为下面的代码紧接着就是 ++ _idx2所以这里只加1.
                _dest[_idx2++]=_buffer[_idx1+3*i];
                _dest[_idx2++]=_buffer[_idx1+3*i+1];
                _dest[_idx2++]=_buffer[_idx1+3*i+2];
        ...
        {//对于8个位
            ...
            {//为1的位要记录0或1
                ...
                _dest[_idx2++] = _contextid>>8;
                _dest[_idx2++]= _contextid&0xff;
                _dest[_idx2++]=0x08; //1bit 00001 000
                if(_tempChar1&0x01)
                {   
                    _dest[_idx2++]=0x01;
                }
